...
class Hand:
    ...
    def scoreHand( self ):
        #remove the Jokers
        jokerCount = self.CardCount(CARDS['J'],0)
        if 'J' in self.cards:
            jokerCount = [ c for c in self.cardCounts if c.cardValue == CARDS['J'] ][0]
            if jokerCount.count == 5:
                score = HANDS['Five of a kind']
                return score

        noJokers = [c for c in self.cardCounts if c.cardValue != CARDS['J'] ]

        mostFreq   = noJokers[0]                              # card with highest count
        secondFreq = noJokers[1] if len(noJokers)>1 else None # and next most frequent

        mostFreqPlusJokers = mostFreq.count + jokerCount.count

        if mostFreqPlusJokers == 5:
            score = HANDS['Five of a kind']
        elif mostFreqPlusJokers == 4:
            score = HANDS['Four of a kind']
        elif mostFreqPlusJokers == 3:
            if secondFreq and secondFreq.count == 2:
                score = HANDS['Full house']
            else:
                score = HANDS['Trips']
        elif mostFreqPlusJokers == 2:
            if secondFreq and secondFreq.count == 2:
